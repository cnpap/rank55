...
const _axiosRetry: AxiosRetry = isNodeEnvironment
  ? require('axios-retry').default
  : axiosRetry;
...
export class OpggDataApi {
  ...
  async getVersions(options: {
    ...
    const versions = await this._http.get<Versions>(
  ...
  async getChampion(options: {
    ...
    OpggNormalModeChampion | OpggArenaModeChampion | OpggArenaModeChampion
  > {
    let {
      id,
      region,
  ...
  async getARAMBalance() {
    return (await this._http.get<OpggARAMBalance>('/api/contents/aram-balance'))
      .data;
  }
}
